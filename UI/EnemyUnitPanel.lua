	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			-- Normal Melee Combat
				...
				iMyDamageInflicted = pMyUnit:GetCombatDamage(iMyStrength, iTheirStrength, pMyUnit:GetDamage() + iTheirFireSupportCombatDamage, false, false, true, pMyUnit:GetMaxHitPoints());
				iTheirDamageInflicted = pMyUnit:GetCombatDamage(iTheirStrength, iMyStrength, pCity:GetDamage(), false, true, false, pMyUnit:GetMaxHitPoints());
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	if (pMyUnit ~= nil) then
		...
		if (iMyStrength > 0) then
			...
			if (iMyDamageInflicted > pTheirUnit:GetMaxHitPoints()) then
				iMyDamageInflicted = pTheirUnit:GetMaxHitPoints();
			end
			if (iTheirDamageInflicted > pMyUnit:GetMaxHitPoints()) then
				iTheirDamageInflicted = pMyUnit:GetMaxHitPoints();
			end
			
			-- now do the health bars
			
			DoUpdateHealthBars(pMyUnit:GetMaxHitPoints(), pTheirUnit:GetMaxHitPoints(), pMyUnit:GetDamage(), pTheirUnit:GetDamage(), iMyDamageInflicted, iTheirDamageInflicted)
